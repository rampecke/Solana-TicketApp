...
app.post("/tickets/:id/buy", async (req, res) => {
  ...
  if (ticket?.event?.endTime <= new Date()) throw new Error("Event has ended");
...
app.get("/collectibles/collection");

app.get("/collectables", (_, res) => {});

app.post("/tickets/:id/claim", async (req, _) => {
  const id = req.params.id as string;
  const walletAddress = req.body.walletAddress as string;

  const [updatedTicket, updatedCollectable] = await prisma.$transaction(
    async (tx) => {
      ...
      if (!ticket) throw new Error("Ticket not found");
      if (ticket?.claimedWalletAddress)
